...
class Staff(SoftDeleteModel):
    From       = models.ForeignKey('auth.User', on_delete=models.CASCADE, related_name = 'Staff_From_Message')
    To         = models.ForeignKey('auth.User',null=True,blank=True, on_delete=models.CASCADE, related_name = 'Staff_To_Message')
    Message    = models.CharField(max_length=20)
    Timestamp  = models.DateTimeField(auto_now_add=True)
    sent_by_fan = models.BooleanField(default=False, null=True)
